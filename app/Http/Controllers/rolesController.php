...
class rolesController extends AppBaseController
{
    ...
    public function __construct(rolesRepository $rolesRepo)
    {
      ...
      /**
     * Show the form for assign a new roles.
     *@param  int $id
     * @return Response
     */
    ...
    //    dd($models);
        ...
        return view('roles.assign')->with('models',$models)->with('role',$id);
    }
    public function assignpost(CreateprivilegesRequest $request)
    {
        $input = $request->all();

        db\privileges::where('role',$input['id'])->delete();

        foreach ($input['role'] as $value) {
            $vals = explode('_', $value, 2);

             $do=  db\privileges::firstOrCreate(['action'=>$vals[1],'model'=>$vals[0],'role'=>$input['id']]);
            //  $do->save();
           // Log::error( $do);
         }

    return view('roles.index')->with('roles', db\roles::all());
}
