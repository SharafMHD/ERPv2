    ...
    @foreach($movementDetails as $movementDetails)
        <tr>
            ...
            <td>{!! $movementDetails->movement_id !!}</td>
                <td width="8%">
                {!! Form::open(['route' => ['inventory.movement_details.destroy', $movementDetails->id], 'method' => 'delete']) !!}
                <div class='btn-group'>
                   @if (auth::user()->GetAuthTable($ucontroller, 'show'))
                    <a href="{!! route('inventory.movement_details.show', [$movementDetails->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-eye-open"></i></a>
                      @endif
                                @if (auth::user()->GetAuthTable($ucontroller, 'edit'))
                    <a href="{!! route('inventory.movement_details.edit', [$movementDetails->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-edit"></i></a>
                         @endif
